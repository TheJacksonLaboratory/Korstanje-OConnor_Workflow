...
filepathp = '/Users/corneb/Documents/MyJax/CS/Projects/Oconnor/genoprobs/all.samples/'
filepaths = '/Users/corneb/Documents/MyJax/CS/Projects/Oconnor/genoprobs/all.samples/'
...
gm <- read_cross2("/Users/corneb/Documents/MyJax/CS/Projects/Oconnor/genoprobs/all.samples/qtl2/gm_sorted.json")
...
#save into pdf
pdf(file = "output/Percent_missing_genotype_data_wcont.pdf", width = 20, height = 20)
...
save(percent_missing,file = "data/percent_missing_id_wcont.RData")
...
bad.sample.cr$Neogen_Sample_ID <- bad.sample.cr$id
...
bad.sample.sex$Neogen_Sample_ID <- bad.sample.sex$Unique.Sample.ID
rownames(bad.sample.sex) <- NULL

bad.sample.sex[c("Neogen_Sample_ID","Sex","Inferred.Sex")] %>%
  kable(escape = F,align = c("ccc"),linesep ="\\hline") %>%
  kable_styling(full_width = F) %>%
  kable_styling("striped", full_width = F) 
...
save(summary.cg,file = "data/summary.cg_wcont.RData")

#pdf(file = "output/Proportion_matching_genotypes_before_removal_of_bad_samples_wcont.pdf", width = 20, height = 20) 
...
m <- maxmarg(pr)
nxo <- count_xo(m)
#save(pr, file = "data/pr_wcont.RData")
#str(pr)
save(m, file = "data/m_wcont.RData")
save(nxo, file = "data/nxo_wcont.RData")

e <- calc_errorlod(gm, pr)
#str(e)
e <- do.call("cbind", e)
save(e, file = "data/e_wcont.RData")

snpg <- predict_snpgeno(gm, m)
snpg <- do.call("cbind", snpg)
save(snpg, file = "data/snpg_wcont.RData")
...
pdf(file = "output/number_crossover_wcont.pdf")
...
totxot$Neogen_Sample_ID <- rownames(totxot)
...
iplot(seq_along(errors_ind), errors_ind, indID=lab,
      chartOpts=list(xlab="Mouse", ylab="Percent genotyping errors", ylim=c(0, 8),
                     axispos=list(xtitle=25, ytitle=50, xlabel=5, ylabel=5)))
save(errors_ind, file = "data/errors_ind_wcont.RData")
...
pdf(file = "output/Percent_genotype_errors_obs_vs_predicted_wcont.pdf",width = 20, height = 20) 
par(pty="s")
err_direct <- rowMeans(snpgn != gobs, na.rm=TRUE)*100
errors_ind_0 <- rowSums(e > 0)[rownames(gm$covar)]/n_typed(gm)*100
par(mar=c(4.1,4.1,0.6, 0.6))
...
samples <- samples[order(samples$Original.Mouse.ID),]
...
save(qc_info, bad.sample, file = "data/qc_info_bad_sample_wcont.RData")
...
save(gm_samqc, file = "data/gm_samqc_wcont.RData")
...
save(e,g,snpg, file = "data/e_g_snpg_samqc_wcont.RData")
#save(e,g, file = "data/e_g_snpg_samqc_wcont.RData")
...
bad$Neogen_Sample_ID <- as.character(bad.sample$id)
...
bad$DO.Generation <- ifelse(bad.sample$generation ==30, 'XX', '')
