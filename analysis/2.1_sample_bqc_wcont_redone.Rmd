---
title: "Step 1: Sample QC [Redone Samples]"
...
filepathp = '/Users/corneb/Documents/MyJax/CS/Projects/Korstanje-Oconnor/haplotype.reconstruction/output/'
filepaths = '/Users/corneb/Documents/MyJax/CS/Projects/Korstanje-Oconnor/haplotype.reconstruction/output/'
#setwd(filepaths)
...
labels <- paste0(as.character(do.call(rbind.data.frame, strsplit(names(percent_missing), "_"))[,6]), " (", round(percent_missing,2), "%)")
iplot(seq_along(percent_missing), percent_missing, indID=labels,
      chartOpts=list(xlab="Mouse", ylab="Percent missing genotype data",
                     ylim=c(0, 70)))

#save into pdf
pdf(file = "output/Percent_missing_genotype_data_wcont_redone.pdf", width = 20, height = 20)
#labels <- as.character(do.call(rbind.data.frame, strsplit(names(totxo), "V01_"))[,2])
labels <- as.character(do.call(rbind.data.frame, strsplit(ind_ids(gm), "_"))[,6])
...
save(percent_missing,file = "data/percent_missing_id_wcont_redone.RData")
...
There are 6 mice that are missing >10% of their genotypes. These samples are:
...
chrx_int <- colMeans(x[chrx,as.character(do.call(rbind.data.frame, strsplit(ind_ids(gm), "_"))[,6])] + y[chrx,as.character(do.call(rbind.data.frame, strsplit(ind_ids(gm), "_"))[,6])], na.rm = T)
chry_int <- colMeans(x[chry,as.character(do.call(rbind.data.frame, strsplit(ind_ids(gm), "_"))[,6])] + y[chry,as.character(do.call(rbind.data.frame, strsplit(ind_ids(gm), "_"))[,6])], na.rm = T)
...
point_colors <- as.character( brocolors("web")[c("red","green", "purple")] )
...
phetX <- phetX[as.character(do.call(rbind.data.frame, strsplit(names(phetX), "_"))[,6]) %in% names(chrx_int)]
names(phetX) <- as.character(do.call(rbind.data.frame, strsplit(names(phetX), "_"))[,6])
...
save(summary.cg,file = "data/summary.cg_wcont_redone.RData")

#pdf(file = "output/Proportion_matching_genotypes_before_removal_of_bad_samples_wcont_redone.pdf", width = 20, height = 20) 
...
#load the intensities.fst_wcont_redone.RData
#load("data/intensities.fst_wcont_redone.RData")

xn <- x[,as.character(do.call(rbind.data.frame, strsplit(ind_ids(gm), "_"))[,6])]
xn <- xn[snps$marker,]
xnm <- rownames(xn)

yn <- y[,as.character(do.call(rbind.data.frame, strsplit(ind_ids(gm), "_"))[,6])]
yn <- yn[snps$marker,]
...
#load the intensities.fst_wcont_redone.RData
#load("data/heh/intensities.fst_wcont_redone.RData")
#X and Y channel
...
int <- int[,intersect(as.character(do.call(rbind.data.frame, strsplit(ind_ids(gm), "_"))[,6]), colnames(int))]
names(percent_missing) <- as.character(do.call(rbind.data.frame, strsplit(names(percent_missing), "_"))[,6])
n <- names(sort(percent_missing[intersect(as.character(do.call(rbind.data.frame, strsplit(ind_ids(gm), "_"))[,6]), colnames(int))], decreasing=TRUE))
...
#load("data/pr_wcont_redone.RData")
pr<-readRDS(paste0(filepaths,"43_DO_Korstanje_Redo__GigaMUGA_genoprobs_36state.rds"))
#load("data/m_wcont_redone.RData")
#load("data/nxo_wcont_redone.RData")

m <- maxmarg(pr)
nxo <- count_xo(m)
#save(pr, file = "data/pr_wcont_redone.RData")
#str(pr)
save(m, file = "data/m_wcont_redone.RData")
save(nxo, file = "data/nxo_wcont_redone.RData")

e <- calc_errorlod(gm, pr)
#str(e)
e <- do.call("cbind", e)
save(e, file = "data/e_wcont_redone.RData")

snpg <- predict_snpgeno(gm, m)
snpg <- do.call("cbind", snpg)
save(snpg, file = "data/snpg_wcont_redone.RData")
...
pdf(file = "output/number_crossover_wcont_redone.pdf")
...
tmp <- cbind(percent_missing=round(percent_missing[as.character(do.call(rbind.data.frame, strsplit(rownames(gm$covar), "_"))[,6])],2), total_xo=totxo)[percent_missing[as.character(do.call(rbind.data.frame, strsplit(rownames(gm$covar), "_"))[,6])] >= 5,]
tmp[order(tmp[,1]),]
...
#load("data/e_wcont_redone.RData")
errors_ind <- rowSums(e>2)[rownames(gm$covar)]/n_typed(gm)*100
lab <- paste0(as.character(do.call(rbind.data.frame, strsplit(names(errors_ind), "_"))[,6]), " (", myround(percent_missing[as.character(do.call(rbind.data.frame, strsplit(rownames(gm$covar), "_"))[,6])],1), "%)")
iplot(seq_along(errors_ind), errors_ind, indID=lab,
      chartOpts=list(xlab="Mouse", ylab="Percent genotyping errors", ylim=c(0, 15),
                     axispos=list(xtitle=25, ytitle=50, xlabel=5, ylabel=5)))
save(errors_ind, file = "data/errors_ind_wcont_redone.RData")
...
#load("data/snpg_wcont_redone.RData")
#snpg <- gm$geno
...
pdf(file = "output/Percent_genotype_errors_obs_vs_predicted_wcont_redone.pdf",width = 20, height = 20) 
par(pty="s")
err_direct <- rowMeans(snpgn != gobs, na.rm=TRUE)*100
errors_ind_0 <- rowSums(e > 0)[rownames(gm$covar)]/n_typed(gm)*100
par(mar=c(4.1,4.1,0.6, 0.6))
...
##percent missing
gm.covar = data.frame(id=as.character(do.call(rbind.data.frame, strsplit(rownames(gm$covar), "_"))[,6]),gm$covar)
...
#genotype errors
qc_info <- merge(qc_info,
                 data.frame(id = as.character(do.call(rbind.data.frame, strsplit(names(errors_ind), "_"))[,6]),
                            genotype_erros = errors_ind,stringsAsFactors = F),by = "id")

##duplicated id to be remove
qc_info$duplicate.id <- ifelse(qc_info$id %in% as.character(do.call(rbind.data.frame, strsplit(summary.cg$remove.id, "_"))[,6]), TRUE,FALSE)
...
bad.sample <- qc_info[qc_info$generation ==1 | qc_info$Number_crossovers <= 200 | qc_info$Number_crossovers >=1000 | qc_info$percent_missing >= 10 | qc_info$genotype_erros >= 1 | qc_info$duplicate.id == TRUE,]

save(qc_info, bad.sample, file = "data/qc_info_bad_sample_wcont_redone.RData")
...
save(gm_samqc, file = "data/gm_samqc_wcont_redone.RData")
...
save(e,g,snpg, file = "data/e_g_snpg_samqc_wcont_redone.RData")
#save(e,g, file = "data/e_g_snpg_samqc_wcont_redone.RData")
...
removed.ids$Neogen_Sample_ID <-  as.character(do.call(rbind.data.frame, strsplit(as.character(removed.ids$Genoprobs_Sample_ID), "_"))[,6])
...
bad$Neogen_Sample_ID <- as.character(do.call(rbind.data.frame, strsplit(as.character(bad$Genoprobs_Sample_ID), "_"))[,6])
