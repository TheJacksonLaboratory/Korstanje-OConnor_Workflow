---
title: "Step 1: Sample QC [With Controls]"
...
filepathp = '/Users/corneb/Documents/MyJax/CS/Projects/Oconnor/genoprobs/3.batches/all.samples/'
filepaths = '/Users/corneb/Documents/MyJax/CS/Projects/Oconnor/genoprobs/3.batches/all.samples/'
#setwd(filepaths)
setwd("/Users/corneb/Documents/MyJax/CS/Projects/Oconnor/workflowr/Korstanje-OConnor_Workflow")
...
#save into pdf
pdf(file = "output/Percent_missing_genotype_data_wcont_3batches.pdf", width = 20, height = 20)
...
save(percent_missing,file = "data/percent_missing_id_wcont_3batches.RData")
...
save(summary.cg,file = "data/summary.cg_wcont_3batches.RData")

#pdf(file = "output/Proportion_matching_genotypes_before_removal_of_bad_samples_wcont_3batches.pdf", width = 20, height = 20) 
...
#load the intensities.fst_wcont_3batches.RData
#load("data/intensities.fst_wcont_3batches.RData")
...
#load the intensities.fst_wcont_3batches.RData
#load("data/heh/intensities.fst_wcont_3batches.RData")
#X and Y channel
...
#load("data/pr_wcont_3batches.RData")
...
#load("data/m_wcont_3batches.RData")
#load("data/nxo_wcont_3batches.RData")

m <- maxmarg(pr)
nxo <- count_xo(m)
#save(pr, file = "data/pr_wcont_3batches.RData")
#str(pr)
save(m, file = "data/m_wcont_3batches.RData")
save(nxo, file = "data/nxo_wcont_3batches.RData")

e <- calc_errorlod(gm, pr)
#str(e)
e <- do.call("cbind", e)
save(e, file = "data/e_wcont_3batches.RData")

snpg <- predict_snpgeno(gm, m)
snpg <- do.call("cbind", snpg)
save(snpg, file = "data/snpg_wcont_3batches.RData")
...
pdf(file = "output/number_crossover_wcont_3batches.pdf")
...
#load("data/e_wcont_3batches.RData")
...
iplot(seq_along(errors_ind), errors_ind, indID=lab,
      chartOpts=list(xlab="Mouse", ylab="Percent genotyping errors", ylim=c(0, 8),
                     axispos=list(xtitle=25, ytitle=50, xlabel=5, ylabel=5)))
save(errors_ind, file = "data/errors_ind_wcont_3batches.RData")
...
#load("data/snpg_wcont_3batches.RData")
#snpg <- gm$geno
...
pdf(file = "output/Percent_genotype_errors_obs_vs_predicted_wcont_3batches.pdf",width = 20, height = 20) 
par(pty="s")
err_direct <- rowMeans(snpgn != gobs, na.rm=TRUE)*100
errors_ind_0 <- rowSums(e > 0)[rownames(gm$covar)]/n_typed(gm)*100
par(mar=c(4.1,4.1,0.6, 0.6))
...
qc_info$duplicate.id <- ifelse(qc_info$id %in% summary.cg$remove.id, TRUE,FALSE)
...
save(qc_info, bad.sample, file = "data/qc_info_bad_sample_wcont_3batches.RData")
...
save(gm_samqc, file = "data/gm_samqc_wcont_3batches.RData")
...
save(e,g,snpg, file = "data/e_g_snpg_samqc_wcont_3batches.RData")
#save(e,g, file = "data/e_g_snpg_samqc_wcont_3batches.RData")
...
bad$Genoprobs_Sample_ID <- as.character(bad.sample$id)
