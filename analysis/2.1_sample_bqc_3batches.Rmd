---
title: "Step 1: Sample QC [Without Controls]"
...
#save into pdf
pdf(file = "output/Percent_missing_genotype_data_3batches.pdf", width = 20, height = 20)
...
save(percent_missing,file = "data/percent_missing_id_3batches.RData")
...
save(summary.cg,file = "data/summary.cg_3batches.RData")

#pdf(file = "output/Proportion_matching_genotypes_before_removal_of_bad_samples_3batches.pdf", width = 20, height = 20) 
...
#load the intensities.fst_3batches.RData
#load("data/intensities.fst_3batches.RData")
...
#load the intensities.fst_3batches.RData
#load("data/heh/intensities.fst_3batches.RData")
#X and Y channel
...
#load("data/m_3batches.RData")
#load("data/nxo_3batches.RData")

m <- maxmarg(pr)
nxo <- count_xo(m)
#save(pr, file = "data/pr_3batches.RData")
#str(pr)
save(m, file = "data/m_3batches.RData")
save(nxo, file = "data/nxo_3batches.RData")

e <- calc_errorlod(gm, pr)
#str(e)
e <- do.call("cbind", e)
save(e, file = "data/e_3batches.RData")

snpg <- predict_snpgeno(gm, m)
snpg <- do.call("cbind", snpg)
save(snpg, file = "data/snpg_3batches.RData")
...
pdf(file = "output/number_crossover_3batches.pdf")
...
iplot(seq_along(errors_ind), errors_ind, indID=lab,
      chartOpts=list(xlab="Mouse", ylab="Percent genotyping errors", ylim=c(0, 8),
                     axispos=list(xtitle=25, ytitle=50, xlabel=5, ylabel=5)))
save(errors_ind, file = "data/errors_ind_3batches.RData")
...
pdf(file = "output/Percent_genotype_errors_obs_vs_predicted_3batches.pdf",width = 20, height = 20) 
par(pty="s")
err_direct <- rowMeans(snpgn != gobs, na.rm=TRUE)*100
errors_ind_0 <- rowSums(e > 0)[rownames(gm$covar)]/n_typed(gm)*100
par(mar=c(4.1,4.1,0.6, 0.6))
...
save(qc_info, bad.sample, file = "data/qc_info_bad_sample_3batches.RData")
...
save(gm_samqc, file = "data/gm_samqc_3batches.RData")
...
save(e,g,snpg, file = "data/e_g_snpg_samqc_3batches.RData")
#save(e,g, file = "data/e_g_snpg_samqc_3batches.RData")
